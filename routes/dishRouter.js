...
dishRouter.use(express.static(__dirname + "/public"));
      ...
      .then(
        (dish) => {
          if (dish != null && dish.comments.id(req.params.commentId) != null) {
            ...
            const userIdDB = dish.comments.id(req.params.commentId).author._id;

            if (userIdReq.equals(userIdDB)) {
              if (req.body.rating) {
              ...
              err = new Error("Operation not allowed");
              err.status = 403;
              return next(err);
            }
          } else if (dish == null) {
